...
class Customers(MethodView):
	...
	def post(self):
		...
		status,customers = Provider.find_customers(gid)
		if not status:
			if customers:
				return jsonify({'error':'Error','msg':customers}), 404
			return jsonify({'error':'Error','msg':'No customers found!'}), 404
		return jsonify({'entities':customers}) , 200
...
customers_api.add_url_rule('/find', view_func=Customers.as_view('customers_api'))
...
class Customer(MethodView):
	...
	def post(self):
		...
		#return success
		status,user  = Provider.create_new_customer(first_name=fname,last_name=lname,phone=phone,address=address,password=password,hen_id=hen_id,gid=gid)
		if not status:
			if user:
...
customers_api.add_url_rule('/new',view_func=Customer.as_view('customer_api'))
