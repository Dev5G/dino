...
from . import salesmen_api
from ....errors import  no_json_error, missing_param_error, response_error
from flask import request, jsonify

class Salesmen(MethodView):
	@jwt_required
	def post(self):
		gid = get_jwt_identity()
		status,salesmen = Provider.find_salesmen(gid)
		if not status:
			if salesmen:
				return jsonify({'error':'Error','msg':salesmen}), 404
			return jsonify({'error':'Error','msg':'No salesmen found!'}), 404
		return jsonify({'entities':salesmen}) , 200
...
salesmen_api.add_url_rule('/find', view_func=Salesmen.as_view('salesmen_api'))


class Salesman(MethodView):
	...
	def post(self):
		...
		#return success
		status,user  = Provider.create_new_salesman(first_name=fname,last_name=lname,phone=phone,address=address,password=password,hen_id=hen_id,gid=gid)
		if not status:
			if user:
				return response_error (title='Request Failed',msg=user), 400
			return response_error (title='Request Failed',msg='There was some problem creating the salesman, please check all fields!'), 400
		return {'salesman':user.json()} , 200

salesmen_api.add_url_rule('/new',view_func=Salesman.as_view('salesman_api'))
