...
@admin.route('/staff/add?salesman', methods=['GET', 'POST'])
@t1_required
def add_salesman():
    title = 'Add Saleman :: New'
    args = request.args
    ...
    if request.method == 'POST':
         ...
         if form.validate_on_submit():
            if Store.add_salesman(user_id=form.user_id.data,store_id=current_user.store_id):
                return {'message' : f'Salesman added successfuly.'}, 201
            else:
                return {'message' : f'Salesman could not be added.'}, 400
    if request.method == 'GET':
        return render_template('admin/staff/add_salesman.html', title=title, form=form)

@admin.route('/salesman/view', methods=['GET', 'POST'])
@t1_required
def view_salesman():
