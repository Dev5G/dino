...
class RattiMethod(Base):
	...
	def find_all(cls, nest_id):
		try:
			return cls.query.filter_by(nest_id=None).union(cls.query.filter_by(nest_id=nest_id)).all()
		except Exception as e:
			print(str(e))
			return None
...
class Product(Base):
	...
	def get_status(cls,id) -> int:
		...
		"""
		status = SalesDetails.find_by_product(id)
		if status:
			return 'Sold'
		return 'Available'
	#@classmethod
	...
	@classmethod
	def find_by_id_and_nest(cls, _id,_nest_id):
		try:
			return cls.query.filter_by(id=_id).filter_by(nest_id=_nest_id).first()
		except Exception as e:
			return None
