...
class _Config(object):
    ...
    def token_location(self):
        locations = current_app.config['OREO_TOKEN_LOCATION']
        if isinstance(locations, str):
            locations = (locations,)
        elif not isinstance(locations, (Sequence, Set)):
            raise RuntimeError('OREO_TOKEN_LOCATION must be a sequence or a set')
        elif not locations:
            raise RuntimeError('OREO_TOKEN_LOCATION must contain at least one '
                               'of "headers", "cookies", "query_string", or "json"')
        for location in locations:
            if location not in ('headers', 'cookies', 'query_string', 'json'):
                raise RuntimeError('OREO_TOKEN_LOCATION can only contain '
    ...
    def header_name(self):
        name = current_app.config['OREO_HEADER_NAME']
        if not name:
            raise RuntimeError("OREO_ACCESS_HEADER_NAME cannot be empty")
        return name

    @property
    def header_type(self):
        return current_app.config['OREO_HEADER_TYPE']

    @property
    def query_string_name(self):
        return current_app.config['OREO_QUERY_STRING_NAME']

    @property
    def access_cookie_name(self):
        return current_app.config['OREO_ACCESS_COOKIE_NAME']

    @property
    def refresh_cookie_name(self):
        return current_app.config['OREO_REFRESH_COOKIE_NAME']

    @property
    def access_cookie_path(self):
        return current_app.config['OREO_ACCESS_COOKIE_PATH']

    @property
    def refresh_cookie_path(self):
        return current_app.config['OREO_REFRESH_COOKIE_PATH']

    @property
    def cookie_secure(self):
        return current_app.config['OREO_COOKIE_SECURE']

    @property
    def cookie_domain(self):
        return current_app.config['OREO_COOKIE_DOMAIN']

    @property
    def session_cookie(self):
        return current_app.config['OREO_SESSION_COOKIE']

    @property
    def cookie_samesite(self):
        return current_app.config['OREO_COOKIE_SAMESITE']

    @property
    def json_key(self):
        return current_app.config['OREO_JSON_KEY']

    @property
    def refresh_json_key(self):
        return current_app.config['OREO_REFRESH_JSON_KEY']

    @property
    def csrf_protect(self):
        return self.jwt_in_cookies and current_app.config['OREO_COOKIE_CSRF_PROTECT']

    @property
    def csrf_request_methods(self):
        return current_app.config['OREO_CSRF_METHODS']

    @property
    def csrf_in_cookies(self):
        return current_app.config['OREO_CSRF_IN_COOKIES']

    @property
    def access_csrf_cookie_name(self):
        return current_app.config['OREO_ACCESS_CSRF_COOKIE_NAME']

    @property
    def refresh_csrf_cookie_name(self):
        return current_app.config['OREO_REFRESH_CSRF_COOKIE_NAME']

    @property
    def access_csrf_cookie_path(self):
        return current_app.config['OREO_ACCESS_CSRF_COOKIE_PATH']

    @property
    def refresh_csrf_cookie_path(self):
        return current_app.config['OREO_REFRESH_CSRF_COOKIE_PATH']
    ...
    def _get_depreciated_csrf_header_name():
        ...
        old_name = current_app.config.get('OREO_CSRF_HEADER_NAME', None)
        if old_name:
            msg = (
                "OREO_CSRF_HEADER_NAME is depreciated. Use OREO_ACCESS_CSRF_HEADER_NAME "
                "or OREO_REFRESH_CSRF_HEADER_NAME instead"
            )
    ...
    def access_csrf_header_name(self):
        return self._get_depreciated_csrf_header_name() or \
               current_app.config['OREO_ACCESS_CSRF_HEADER_NAME']

    @property
    def refresh_csrf_header_name(self):
        return self._get_depreciated_csrf_header_name() or \
               current_app.config['OREO_REFRESH_CSRF_HEADER_NAME']

    @property
    def csrf_check_form(self):
        return current_app.config['OREO_CSRF_CHECK_FORM']

    @property
    def access_csrf_field_name(self):
        return current_app.config['OREO_ACCESS_CSRF_FIELD_NAME']

    @property
    def refresh_csrf_field_name(self):
        return current_app.config['OREO_REFRESH_CSRF_FIELD_NAME']

    @property
    def access_expires(self):
        delta = current_app.config['OREO_ACCESS_TOKEN_EXPIRES']
        ...
        if delta is not False:
            try:
                ...
                err = (
                    "must be able to add OREO_ACCESS_TOKEN_EXPIRES to datetime.datetime"
                )
    ...
    def refresh_expires(self):
        delta = current_app.config['OREO_REFRESH_TOKEN_EXPIRES']
        if type(delta) is int:
            delta = datetime.timedelta(seconds=delta)
        if delta is not False:
            try:
                ...
                err = (
                    "must be able to add OREO_REFRESH_TOKEN_EXPIRES to datetime.datetime"
                )
                raise_from(RuntimeError(err), e)
        return delta
    ...
    @property
    def algorithm(self):
        return current_app.config['OREO_ALGORITHM']

    @property
    def decode_algorithms(self):
        algorithms = current_app.config['OREO_DECODE_ALGORITHMS']
        if not algorithms:
    ...
    def blacklist_enabled(self):
        return current_app.config['OREO_BLACKLIST_ENABLED']

    @property
    def blacklist_checks(self):
        check_type = current_app.config['OREO_BLACKLIST_TOKEN_CHECKS']
        if isinstance(check_type, str):
            check_type = (check_type,)
        elif not isinstance(check_type, (Sequence, Set)):
            raise RuntimeError('OREO_BLACKLIST_TOKEN_CHECKS must be a sequence or a set')
        for item in check_type:
            if item not in ('access', 'refresh'):
                err = 'OREO_BLACKLIST_TOKEN_CHECKS must be "access" or "refresh"'
    ...
    def _secret_key(self):
        key = current_app.config['OREO_SECRET_KEY']
        if not key:
            key = current_app.config.get('SECRET_KEY', None)
            if not key:
                raise RuntimeError('OREO_SECRET_KEY or flask SECRET_KEY '
    ...
    def _public_key(self):
        key = current_app.config['OREO_PUBLIC_KEY']
        if not key:
            raise RuntimeError('OREO_PUBLIC_KEY must be set to use '
    ...
    def _private_key(self):
        key = current_app.config['OREO_PRIVATE_KEY']
        if not key:
            raise RuntimeError('OREO_PRIVATE_KEY must be set to use '
                               'asymmetric cryptography algorithm '
                               '"{}"'.format(self.algorithm))
        return key
    ...
    def identity_claim_key(self):
        return current_app.config['OREO_IDENTITY_CLAIM']

    @property
    def user_claims_key(self):
        return current_app.config['OREO_USER_CLAIMS']

    @property
    def user_claims_in_refresh_token(self):
        return current_app.config['OREO_CLAIMS_IN_REFRESH_TOKEN']
    ...
    def error_msg_key(self):
        return current_app.config['OREO_ERROR_MESSAGE_KEY']
    ...
    @property
    def audience(self):
        return current_app.config['OREO_DECODE_AUDIENCE']

    @property
    def issuer(self):
        return current_app.config['OREO_DECODE_ISSUER']

    @property
    def leeway(self):
        return current_app.config['OREO_DECODE_LEEWAY']
