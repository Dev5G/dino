...
class JWTManager(object):
    ...
    def init_app(self, app):
        ...
        if not hasattr(app, 'extensions'):   # pragma: no cover
            app.extensions = {}
        app.extensions['flask-oreo-jwt'] = self
    ...
    def _set_default_configuration_options(app):
        ...
        app.config.setdefault('OREO_TOKEN_LOCATION', ('headers',))

        # Options for JWTs when the TOKEN_LOCATION is headers
        app.config.setdefault('OREO_HEADER_NAME', 'Oreo_Authorization')
        app.config.setdefault('OREO_HEADER_TYPE', 'Bearer')

        # Options for JWTs then the TOKEN_LOCATION is query_string
        app.config.setdefault('OREO_QUERY_STRING_NAME', 'oreo-jwt')

        # Option for JWTs when the TOKEN_LOCATION is cookies
        app.config.setdefault('OREO_ACCESS_COOKIE_NAME', 'oreo_access_token_cookie')
        app.config.setdefault('OREO_REFRESH_COOKIE_NAME', 'oreo_refresh_token_cookie')
        app.config.setdefault('OREO_ACCESS_COOKIE_PATH', '/')
        app.config.setdefault('OREO_REFRESH_COOKIE_PATH', '/')
        app.config.setdefault('OREO_COOKIE_SECURE', False)
        app.config.setdefault('OREO_COOKIE_DOMAIN', None)
        app.config.setdefault('OREO_SESSION_COOKIE', True)
        app.config.setdefault('OREO_COOKIE_SAMESITE', None)

        # Option for JWTs when the TOKEN_LOCATION is json
        app.config.setdefault('OREO_JSON_KEY', 'oreo_access_token')
        app.config.setdefault('OREO_REFRESH_JSON_KEY', 'oreo_refresh_token')

        # Options for using double submit csrf protection
        app.config.setdefault('OREO_COOKIE_CSRF_PROTECT', True)
        app.config.setdefault('OREO_CSRF_METHODS', ['POST', 'PUT', 'PATCH', 'DELETE'])
        app.config.setdefault('OREO_ACCESS_CSRF_HEADER_NAME', 'Oreo-X-CSRF-TOKEN')
        app.config.setdefault('OREO_REFRESH_CSRF_HEADER_NAME', 'Oreo-X-CSRF-TOKEN')
        app.config.setdefault('OREO_CSRF_IN_COOKIES', True)
        app.config.setdefault('OREO_ACCESS_CSRF_COOKIE_NAME', 'oreo_csrf_access_token')
        app.config.setdefault('OREO_REFRESH_CSRF_COOKIE_NAME', 'oreo_csrf_refresh_token')
        app.config.setdefault('OREO_ACCESS_CSRF_COOKIE_PATH', '/')
        app.config.setdefault('OREO_REFRESH_CSRF_COOKIE_PATH', '/')
        app.config.setdefault('OREO_CSRF_CHECK_FORM', False)
        app.config.setdefault('OREO_ACCESS_CSRF_FIELD_NAME', 'oreo_csrf_token')
        app.config.setdefault('OREO_REFRESH_CSRF_FIELD_NAME', 'oreo_csrf_token')

        # How long an a token will live before they expire.
        app.config.setdefault('OREO_ACCESS_TOKEN_EXPIRES', datetime.timedelta(minutes=15))
        app.config.setdefault('OREO_REFRESH_TOKEN_EXPIRES', datetime.timedelta(days=30))
        ...
        app.config.setdefault('OREO_ALGORITHM', 'HS256')

        # What algorithms are allowed to decode a token
        app.config.setdefault('OREO_DECODE_ALGORITHMS', None)
        ...
        # if this is not set.
        app.config.setdefault('OREO_SECRET_KEY', None)

        # Keys to sign JWTs with when use when using an asymmetric
        # (public/private key) algorithm, such as RS* or EC*
        app.config.setdefault('OREO_PRIVATE_KEY', None)
        app.config.setdefault('OREO_PUBLIC_KEY', None)

        # Options for blacklisting/revoking tokens
        app.config.setdefault('OREO_BLACKLIST_ENABLED', False)
        app.config.setdefault('OREO_BLACKLIST_TOKEN_CHECKS', ('access', 'refresh'))

        app.config.setdefault('OREO_IDENTITY_CLAIM', 'identity')
        app.config.setdefault('OREO_USER_CLAIMS', 'user_claims')
        app.config.setdefault('OREO_DECODE_AUDIENCE', None)
        app.config.setdefault('OREO_DECODE_ISSUER', None)
        app.config.setdefault('OREO_DECODE_LEEWAY', 0)

        app.config.setdefault('OREO_CLAIMS_IN_REFRESH_TOKEN', False)

        app.config.setdefault('OREO_ERROR_MESSAGE_KEY', 'msg')

    def user_claims_loader(self, callback):
        """
        This decorator sets the callback function for adding custom claims to an
        access token when :func:`~flask_oreo_jwt.create_access_token` is
        ...
        *HINT*: The callback function must be a function that takes only **one** argument,
        ...
        :func:`~flask_oreo_jwt.create_access_token`, and returns the custom
    ...
    def user_identity_loader(self, callback):
        ...
        :func:`~flask_oreo_jwt.create_access_token` and
        :func:`~flask_oreo_jwt.create_refresh_token`. By default, this will
        return the unmodified object that is passed in as the `identity` kwarg
        to the above functions.

        *HINT*: The callback function must be a function that takes only **one** argument,
        which is the object passed into
        :func:`~flask_oreo_jwt.create_access_token` or
        :func:`~flask_oreo_jwt.create_refresh_token`, and returns the
    ...
    def needs_fresh_token_loader(self, callback):
        ...
        the :func:`~flask_oreo_jwt.fresh_jwt_required` decorator. The
    ...
    def user_loader_callback_loader(self, callback):
        ...
        *HINT*: The callback must take **one** argument which is the identity JWT
        ...
        then be accessed via the :attr:`~flask_oreo_jwt.current_user` LocalProxy
        ...
        `None`, the :meth:`~flask_oreo_jwt.JWTManager.user_loader_error_loader`
        will be called.
        """
    ...
    def user_loader_error_loader(self, callback):
        ...
        :meth:`~flask_oreo_jwt.JWTManager.user_loader_callback_loader`
    ...
    def claims_verification_loader(self, callback):
        ...
        :meth:`~flask_oreo_jwt.JWTManager.claims_verification_failed_loader`
        decorator.
    ...
    def claims_verification_failed_loader(self, callback):
        ...
        the :meth:`~flask_oreo_jwt.JWTManager.claims_verification_loader`
